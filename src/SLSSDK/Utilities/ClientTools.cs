  ...
  internal class LogClientTools
  {
    ...
    public static void ResponseErrorCheck(ServiceResponse response, ServiceCredentials credentials)
    {
      if (response.StatusCode != HttpStatusCode.OK)
      {
        ...
        throw new LogException(body[LogConsts.NAME_ERROR_CODE].ToString(),
          body[LogConsts.NAME_ERROR_MESSAGE].ToString(), requestId);
      }
    }
    ...
    private static string GetRequestString(IEnumerable<KeyValuePair<string, string>> parameters, String kvDelimiter,
      String separator)
    {
    ...
    public static String Signature(IDictionary<String, String> headers, IDictionary<String, String> paramDic,
      HttpMethod method, String resource, String accessKeyId, String accessKey)
    {
    ...
    public static JArray ParserResponseToJArray(Stream response)
    {
      using (response)
      {
        ...
        {
          throw new LogException("LOGBadResponse",
            "There is not enough memory to continue the execution of parsing the response data : ", e);
        }
        finally
        {
          sr.Close();
